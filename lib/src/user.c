...
void board_print(Sudoku *s) {   //оголошення функції виведення дошки
    ...
    for (int i = 1; i <= SIZE_BOARD; i++) {       //виведення номерів стовпців
        printf("%s%d%s ", CYAN, i, RESET);      //виведення номерів у кольорі
        if (i % 3 == 0 && i < SIZE_BOARD) printf("  ");   //якщо це 3 або 6 стовпець, розділяємо візуально блоки 3х3
    }

    printf("\n");
    printf("  ┏━━━━━━━┳━━━━━━━┳━━━━━━━┓\n");
    
    for (int i = 0; i < SIZE_BOARD; i++) {            //виведення рядків
        printf("%s%d%s ┃ ", CYAN, i + 1, RESET);       //колір
        for (int j = 0; j < SIZE_BOARD; j++) {        //виведення клітинок у рядку
            ...
            if ((j + 1) % 3 == 0 && j < SIZE_BOARD - 1) printf("┃ ");     //вертикальне розділення після 3 і 6 стовпців
        }
        printf("┃\n");      //кінець рядка

        if ((i + 1) % 3 == 0 && i < SIZE_BOARD - 1) printf("  ┣━━━━━━━╋━━━━━━━╋━━━━━━━┫\n");  //після 3 да 6 рядків, розділюємо дошку
    }
    printf("  ┗━━━━━━━┻━━━━━━━┻━━━━━━━┛\n");        //нижня рамка
}